using System;
using System.Collections.Generic;
using System.Data;
using System.Data.Entity;
using System.Linq;
using System.Net;
using System.Web;
using System.Web.Mvc;
using MVCSIMPLE.Models;

namespace MVCSIMPLE.Controllers
{
    public class tasknamesController : Controller
    {
        private TODOListEntities db = new TODOListEntities();

        // GET: tasknames
        public ActionResult Index()
        {
            return View(db.tasknames.ToList());
        }

        // GET: tasknames/Details/5
        public ActionResult Details(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            taskname taskname = db.tasknames.Find(id);
            if (taskname == null)
            {
                return HttpNotFound();
            }
            return View(taskname);
        }

        // GET: tasknames/Create
        public ActionResult Create()
        {
            return View();
        }
        public ActionResult Completed()
        {
            return View();
        }

        // POST: tasknames/Create
        // To protect from overposting attacks, please enable the specific properties you want to bind to, for 
        // more details see https://go.microsoft.com/fwlink/?LinkId=317598.
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Create([Bind(Include = "id,Taskname1")] taskname taskname)
        {
            if (ModelState.IsValid)
            {
                db.tasknames.Add(taskname);
                db.SaveChanges();
                return RedirectToAction("Index");
            }

            return View(taskname);
        }

        // GET: tasknames/Edit/5
        public ActionResult Edit(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            taskname taskname = db.tasknames.Find(id);
            if (taskname == null)
            {
                return HttpNotFound();
            }
            return View(taskname);
        }

        // POST: tasknames/Edit/5
        // To protect from overposting attacks, please enable the specific properties you want to bind to, for 
        // more details see https://go.microsoft.com/fwlink/?LinkId=317598.
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "id,Taskname1")] taskname taskname)
        {
            if (ModelState.IsValid)
            {
                db.Entry(taskname).State = EntityState.Modified;
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(taskname);
        }

        // GET: tasknames/Delete/5
        public ActionResult Delete(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            taskname taskname = db.tasknames.Find(id);
            if (taskname == null)
            {
                return HttpNotFound();
            }
            return View(taskname);
        }

        // POST: tasknames/Delete/5
        [HttpPost, ActionName("Delete")]
        [ValidateAntiForgeryToken]
        public ActionResult DeleteConfirmed(int id)
        {
            taskname taskname = db.tasknames.Find(id);
            db.tasknames.Remove(taskname);
            db.SaveChanges();
            return RedirectToAction("Index");
        }

        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                db.Dispose();
            }
            base.Dispose(disposing);
        }
    }
}
